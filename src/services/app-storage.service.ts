...
export class AppStorage {
  static getMapping = async (fallback?: Mapping): Promise<Mapping> => {
    return AsyncStorage.getItem(MAPPING_KEY).then((mapping: any) => {
      return mapping || fallback;
    });
  };

  static getTheme = async (fallback?: Theme): Promise<Theme> => {
